...
samp1%>%
summarise(xames=mean(area), pop_med=median(area), sdames=sd(area), pop_iqr=IQR(area), pop_min=min(area), pop_max=max(area), pop_q1=quantile(area,0.25), pop_q3=quantile(area, 0.75))
```
...
sample_means_small <- ames %>%
                    rep_sample_n(.,tbl= "ames", size = 10, reps = 25, replace = TRUE) %>%
                   summarise(x_bar = mean(area))
ames %>%
  sample_n(size = 10) %>%
  summarise(x_bar = mean(area))
